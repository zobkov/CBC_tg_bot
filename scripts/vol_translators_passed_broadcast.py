#!/usr/bin/env python3
"""
Broadcast script — Volunteer selection: Translators, PASSED.

Sends congratulations + confirmation buttons (Yes / No) to translators
...
SOURCE_CSV = "КБК 26 Отбор волонтеров - переводчики_прошли.csv"

_MESSAGE_TEXT = (
    "<b>Привет!</b>\n\n"
    "Поздравляем тебя с успешным прохождением отбора на роль переводчика "
    "на форуме КБК!\n\n\n"
...
def setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("vol_translators_passed_broadcast")
    logger.setLevel(logging.INFO)
    fmt = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s", datefmt="%H:%M:%S")
    ch = logging.StreamHandler()
    ch.setFormatter(fmt)
    fh = logging.FileHandler(
        f"storage/vol_translators_passed_broadcast_{timestamp}.log", encoding="utf-8"
    )
    fh.setFormatter(fmt)
...
def create_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="Да", callback_data="vol_trans_confirm_yes"),
                InlineKeyboardButton(
                    text="К сожалению, нет", callback_data="vol_trans_confirm_no"
                ),
            ]
        ]
    )
...
def preview(logger: logging.Logger) -> None:
    ...
    print(_MESSAGE_TEXT)
    print("\n🔘 Buttons:")
    print("   [Да]  (callback: vol_trans_confirm_yes)")
    print("   [К сожалению, нет]  (callback: vol_trans_confirm_no)")
...
async def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("VOL TRANSLATORS PASSED BROADCAST")
    logger.info("=" * 60)
