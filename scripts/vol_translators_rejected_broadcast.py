#!/usr/bin/env python3
"""
Broadcast script — Volunteer selection: Translators, REJECTED.

Sends a rejection message (no buttons) to translators listed in the CSV.
...
SOURCE_CSV = "КБК 26 Отбор волонтеров - переводчики_отказ.csv"
...
def setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("vol_translators_rejected_broadcast")
    logger.setLevel(logging.INFO)
    fmt = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s", datefmt="%H:%M:%S")
    ch = logging.StreamHandler()
    ch.setFormatter(fmt)
    fh = logging.FileHandler(
        f"storage/vol_translators_rejected_broadcast_{timestamp}.log", encoding="utf-8"
    )
    fh.setFormatter(fmt)
...
async def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("VOL TRANSLATORS REJECTED BROADCAST")
    logger.info("=" * 60)
