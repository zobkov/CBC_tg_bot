...
def setup_logging() -> logging.Logger:
    ...
    logger = logging.getLogger("vol_general_passed_broadcast")
    logger.setLevel(logging.INFO)
    fmt = logging.Formatter("%(asctime)s - %(levelname)s - %(message)s", datefmt="%H:%M:%S")
    ch = logging.StreamHandler()
    ch.setFormatter(fmt)
    fh = logging.FileHandler(
        f"storage/vol_general_passed_broadcast_{timestamp}.log", encoding="utf-8"
    )
    fh.setFormatter(fmt)
...
async def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("VOL GENERAL PASSED BROADCAST")
    logger.info("=" * 60)
