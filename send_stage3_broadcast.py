...
class Stage3BroadcastService:
    ...
    async def send_approval_notification(self, user_id: int, position_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            message = f"""Наплыв желающих пройти собеседование оказался таким, что бот решил тоже немного «вздохнуть» и уйти на тех паузу!

Если у тебя не получается выбрать время для интервью — всё в порядке. Система восстановит работу в ближайший час, и можно будет спокойно выбрать удобный слот.

Мы заранее подготовили около 150 временных слотов с 9 по 12 октября, чтобы каждый смог найти своё идеальное время. Главное — успей сделать выбор до 8 октября, 23:59 

Спасибо, что сохраняешь спокойствие и остаёшься с нами. Такой ажиотаж — лучший знак, что команда КБК растёт и вдохновляет!
"""
...
"""
    ...
    async def send_stage3_broadcasts(self, dry_run: bool = True, test_user_ids: List[int] = None, approved_only: bool = False) -> Dict[str, int]:
        ...
        if test_user_ids:
            ...
            if approved_only:
                logger.info("✅ Starting APPROVED ONLY broadcast...")
            else:
        ...
        # Filter only approved users if approved_only mode
        if approved_only:
            users = [user for user in users if user['approved'] and int(user['approved']) > 0]
            logger.info(f"✅ Filtered to {len(users)} approved users only")
        ...
        for user in users:
            ...
            try:
                if dry_run:
                    ...
                    elif not approved_only:  # Only count rejections if not in approved_only mode
                    ...
                    if approved > 0:
                        ...
                        if position_info:
                            ...
                            stats["errors"] += 1
                    elif not approved_only:  # Only send rejections if not in approved_only mode
                        # User is rejected - send rejection with feedback option
                        success = await self.send_rejection_notification(user_id)
                        if success:
                            stats["rejected_sent"] += 1
                        else:
                            stats["errors"] += 1                # Small delay between messages
                if not dry_run:
...
async def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        dry_run = False
        approved_only = True
        logger.warning("✅ APPROVED ONLY MODE - Real messages only to approved users!")
    else:
    ...
    try:
        ...
        # Filter only approved users if approved_only mode
        if approved_only:
            users_data = [user for user in users_data if user['approved'] and int(user['approved']) > 0]
        ...
        if test_user_ids:
            ...
            print("✅ MODE: APPROVED ONLY (real messages only to approved users)")
        elif dry_run:
        ...
        if not dry_run:
            ...
            if test_user_ids:
                ...
                print("✅ APPROVED ONLY - сообщения только одобренным пользователям!")
            else:
        ...
        if dry_run and not test_user_ids and not approved_only:
            print("\n💡 Available modes:")
            ...
            print("   python3 send_stage3_broadcast.py --approved # Real broadcast only to approved users")
