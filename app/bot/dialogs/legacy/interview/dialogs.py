...
interview_dialog = Dialog(
    ...
    Window(
        Multi(
            ...
            Format("<b>Ты прошёл на позицию:</b>\n{approved_position_title}\n", when="has_approval"),
            Const("❌ К сожалению, ты не прошёл во третий этап отбора.", when=~F["has_approval"]),
            Const("Теперь тебе нужно выбрать удобный день и время, чтобы пообщаться с руководителем отдела. Определись с выбором до 23:59 8 октября.\n", when="has_approval"),
            Format("📅 <b>Твоё собеседование состоится</b> {booking_date} в {booking_time}", 
    ...
    Window(
        Multi(
            Const("📅 Мы постарались сделать расписание максимально гибким, чтобы тебе было удобно выбрать подходящее время — просто отметь день и время, и мы свяжемся с тобой для подтверждения интервью.\n\n<b>Сначала выбери день для собеседования:</b>\n"),
            sep="\n"
        ),
        Group(
            Select(
    ...
    Window(
        Multi(
            ...
            Format("Доступно слотов: {total_slots}"),
            sep="\n"
        ),
        ScrollingGroup(
            Select(
    ...
    Window(
        Multi(
            ...
            Const("<b>Не забудь подтвердить свою запись:</b>"),
            sep="\n"
        ),
        Row(
            Button(
    ...
    Window(
        Multi(
            Const("🎉 <b>Отлично, ты в расписании!</b>\n\nВсё готово к встрече — время подтверждено, и совсем скоро с тобой свяжется руководитель отдела, чтобы пообщаться о твоём опыте, идеях и взглядах! \n\nЕсли что-то изменится или появятся вопросы — просто напиши в поддержку, мы всегда рады помочь 🫶🏻"),
            sep=""
        ),
        Row(
            Start(
    ...
    Window(
        Multi(
            ...
            Const("<b>Не забудь подтвердить перенос записи:</b>"),
            sep="\n"
        ),
        Row(
            Button(
    ...
    Window(
        Multi(
            Const("❌ <b>Отмена записи</b>\n"),
            Format("📅 Твоя запись: {booking_date} в {booking_time}\n"),
            Const("Ты действительно хочешь отменить собеседование? Не переживай — после отмены ты сможешь выбрать другое удобное время!\n"),
            sep="\n"
        ),
        Row(
            Button(
