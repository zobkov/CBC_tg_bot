...
from aiogram_dialog.widgets.kbd import Button, Cancel, Column, Group, Select
...
_INTRO_TEXT = (
    "大家好! Мы подготовили короткий квиз, чтобы проверить, насколько хорошо ты знаешь "
    "культуру и традиции Поднебесной, и познакомить тебя с нашим проектом.\n\n"
    "И это только начало  👀\n\n"
    "Совсем скоро — лекции от экспертов, новые челленджи, онлайн-встречи с ребятами из "
    "разных регионов и другие сюрпризы от команды КБК!\n\n"
    "Подписывайся на наши соцсети — впереди ещё много всего интересного!\n\n"
    "Не тяни — жми «начать квиз» и вперёд 🚀"
)

_EDUCATION_PROMPT = (
    "Укажи школу / университет, класс / курс.\n\n"
    "Например: <b>ГБОУ СОШ №241, 11 класс</b>  /  "
    "<b>ВШМ СПбГУ, 1 курс, Международный Менеджмент.</b>"
)
...
_LEARNER_TEXT = (
    ...
    "Попробуй пройти квиз ещё раз, и помни, что путь к успеху начинается с интереса, "
...
quiz_dod_dialog = Dialog(
    ...
    Window(
        Multi(
            Format(_PASSED_TEXT, when="passed_threshold"),
            Format(_LEARNER_TEXT, when=~F["passed_threshold"]),
