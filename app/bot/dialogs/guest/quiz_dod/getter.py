"""Dialog data getters and helpers for the DoD quiz flow."""
...
async def _ensure_best_score(dialog_manager: DialogManager) -> None:
    ...
    try:
        ...
        logger.exception(
            "[QUIZ_DOD] Failed to load best score for user=%s: %s",
...
async def get_intro_data(dialog_manager: DialogManager, **_kwargs: Any) -> dict[str, Any]:
...
async def get_question_data(dialog_manager: DialogManager, **_kwargs: Any) -> dict[str, Any]:
...
async def get_results_data(dialog_manager: DialogManager, **_kwargs: Any) -> dict[str, Any]:
    ...
    if certificate_requested is None:
        ...
        if db and user:
            try:
                ...
                logger.exception(
                    "[QUIZ_DOD] Failed to load certificate status for user=%s: %s",
                    getattr(user, "id", "unknown"),
                    exc,
                )
